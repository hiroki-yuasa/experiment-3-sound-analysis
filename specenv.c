...
/* plot the result */
    ...
    for (i = 0; i < framelen; i++)
    {
        ...
        printf("%f %lf\n", (float)i * 16000 / framelen, spec);
    }

#else
    for (i = 0; i < framelen; i++)
    {
        Xr[i] = log10((Xr[i] * Xr[i] + Xi[i] * Xi[i]) / framelen);
        Xi[i] = 0.0;
    }
    ...
    for (i = cutfreq; i < framelen - cutfreq; i++)
    {
        xr[i] = 0.0;
        xi[i] = 0.0;
    }

    /*FFT trial after high range cut */
    FFT(xr, xi, Xr, Xi, framelen);
